...
int8_t nrf24_read(uint8_t *buffer, uint8_t size)
{
	...
	status = nrf24_read_reg_status(); // Read the status register
	if (status & nRF24_MASK_RX_DR) {
		// RX_DR bit set (Data ready RX FIFO interrupt)
		result = ((status & 0x0e) >> 1); // Pipe number
		if (result < 6) {
